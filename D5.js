...
const OnlyLetters = (x) => x = x.replace(/\d+/g,"")
...
//console.log(IsThisAnEmail("arturkemmerer@gmx.net"))
...
var date2 = new Date(WhatDayIsIt()); 
...
//console.log("How Many days have passed: " + HowManyDays("10/01/2020"))
...
function IsTodayMyBDay() {
  ...
  let bday = new Date("04/23/1999")
  today.setYear(0);
  bday.setYear(0);
  if(today.valueOf() === bday.valueOf()){
    return true
  } else {
    return false
  }
...
function OlderMovie() {
  let result = movies.find(obj => {
    return obj.Year === movies.reduce((min, p) => p.Year < min ? p.Year : min, movies[0].Year)
  }) 
  return result
}
...
function countMovies() {
  x = "Number of Movies: " + movies.length
  return x
}
/* Ex.14
...
*/
function OnlyTitles() {
  let result = movies.map(a => a.Title);
  return result
}
